...
public class RestoreUserDataTask : IScheduledTask
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Everything that decides whether a restore is safe lives in here, where
        // a test can inject the faults it guards against instead of waiting for a
        // server to produce one.
        var applier = new PlannedWriteApplier(
            _userManager, _libraryManager, writer, reader, collector, options, ownership, _logger);
